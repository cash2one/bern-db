...
quotes_db = []
with open("quotes_db.json") as db:
    quotes_db = json.load(db)
...
def quotes_by_tag(tag):
    return [quote for quote in quotes_db if tag in quote["tags"]]

@app.route("/v0/quotes", methods=["GET"])
def get_quotes():
    return jsonify({"quotes": [make_public_quote(quote) for quote in quotes_db]})

@app.route("/v0/quotes/<int:quote_id>", methods=["GET"])
def get_quote(quote_id):
    quote = [quote for quote in quotes_db if quote["id"] == quote_id]
    if len(quote) == 0:
        abort(404)
